...
export const isImport = line =>
  line.includes('import') || line.includes('use strict')

export const isConst = line => line.includes('const') || line.includes('let')

export const isFunction = line => line.includes('createMainWindow')

export const isLoadUrl = line => line.includes('loadURL')

export const isElse = line => line.includes('else')

export const isListener = line => line.includes('app.on')

export const isElseEnd = line => line.trim() === '}'

export const isReturn = line => line.trim().indexOf('return') === 0

export const isFunctionEnd = line => line.trimRight() === '}'

export const isInFunction = line => !isReturn(line) && !isFunctionEnd(line)

export const isBlank = line => !line || line.trim().length === 0

export const isCompleteLine = line =>
  line && line.trim().charAt(line.trim().length - 1) === ';'

export const isNotLineEnd = line =>
  line && line.trim().charAt(line.trim().length - 1) !== ';'

export const isInCompleteLine = line =>
  line && isConst(line) && line.trim().charAt(line.trim().length - 1) === '{'
...
export const merge = (base, patch) => {
  ...
  read(`./templates/${patch}.js`).forEach((line, i) => {
    ...
    if (lookForSemicolon) {
      console.log('waiting for semicolon')
      pushPatch(line)
